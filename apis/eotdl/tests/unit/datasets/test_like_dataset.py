...
from ....src.usecases.datasets.LikeDataset import LikeDataset
...
def test_like_dataset(user):
    ...
    like(inputs)
    db_repo.exists.assert_called_once_with('datasets', '123')
    db_repo.increase_counter.assert_called_once_with('datasets', '_id', '123', 'likes', 1)
    db_repo.append_to_list.assert_called_once_with('users', 'uid', '123', 'liked_datasets', '123')
    user['liked_datasets'] = ['123']
    like(inputs)
    db_repo.increase_counter.assert_called_with('datasets', '_id', '123', 'likes', -1)
    db_repo.remove_from_list.assert_called_once_with('users', 'uid', '123', 'liked_datasets', '123')