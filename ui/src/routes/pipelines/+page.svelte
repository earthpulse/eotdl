<script>
	...
	const load = async () => {
		await pipelines.retrieve(fetch);
		loading = false;
	...
	let filtered_pipelines = $state(null);
	$effect(() => {
		let base_pipelines = $pipelines.data || []; // Start with all data or an empty array

		// Filter by tags
		let pipelines_after_tags = base_pipelines.filter((model) => {
		...
		if (show_liked && auth.user) {
			final_pipelines = pipelines_after_name.filter((model) =>
				auth.user.liked_pipelines.includes(model.id),
			);
		} else {
			final_pipelines = pipelines_after_name;
		}

		// Assign the final result
		filtered_pipelines = final_pipelines;
	});
...
<div class="w-full flex flex-col items-center justify-between h-full grow">
	...
	>
		<div class="grid grid-cols-1 sm:grid-cols-[250px_auto] gap-8 w-full">
			<div class="flex flex-col w-full">
				<div class="flex flew-row justify-between text-3xl">
					<h1 class="font-bold">Pipelines</h1>
					<p class="text-gray-400">
						{loading ? "" : filtered_pipelines?.length}
					</p>
				</div>
				<input
		...
		{:else if filtered_pipelines?.length > 0}
			<div class="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full mt-3">
				{#each filtered_pipelines as pipeline, i}
					<Card
						data={pipeline}
						link="pipelines"
						liked={auth.user?.liked_pipelines?.includes(
							pipeline.id,
						)}
						tags={data.tags}
					/>
		...
		{:else}
			<p class="text-gray-400 text-center">No pipelines found</p>
		{/if}
	</div>
	<!-- <PipelinesLeaderboard /> -->
</div>
