...
@pytest.fixture
def user():
    return {'uid': '123', 'email': 'test1', 'name': 'test1', 'picture': 'test1', 'dataset_count': 1}

@pytest.fixture
def dataset():
    return {'id': '123', 'name': 'test', 'uid': '123'}
...
def test_delete_dataset(user, dataset):
    ...
    os_repo.delete.assert_called_once_with('123')
    db_repo.delete.assert_called_once_with('datasets', '123')
    db_repo.increase_counter.assert_called_once_with('users', 'uid', '123', 'dataset_count', -1)