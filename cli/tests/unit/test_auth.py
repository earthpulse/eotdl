...
from ...eotdl_cli.src.usecases.auth.Auth import Auth
from ...eotdl_cli.src.errors.auth import LoginError, AuthTimeOut
from ...eotdl_cli.src.usecases.auth.IsLogged import IsLogged
from ...eotdl_cli.src.usecases.auth.Logout import Logout
...
def test_auth():
	...
	inputs = Auth.Inputs()
	api_repo.login.return_value = mock.Mock(status_code=200, json=mock.Mock(return_value={'code': 123, 'login_url': '123123'}))
	token_data = {'id_token': 'lkajshdflkjahsldkfjhalskjdfhlkasjdhf'}
	api_repo.token.return_value = mock.Mock(status_code=200, json=mock.Mock(return_value=token_data))
	repo.save_creds.return_value = 'creds_path'
	repo.decode_token.return_value = {'uid': 'test', 'name': 'test name', 'email': 'test email', 'picture': 'test picture'}
	outputs = auth(inputs)
...
def test_auth_fails_if_auth_timeout():
	repo = mock.Mock()
	api_repo = mock.Mock()
	auth = Auth(repo, api_repo, max_t=0.1, interval=0.01) # will try to login 10 times, then timeout
	inputs = Auth.Inputs()
	api_repo.login.return_value = mock.Mock(status_code=200, json=mock.Mock(return_value={'code': 123, 'login_url': '123123'}))
