...
path = 'data/SatellogicDataset'
dst_path = './data/'
data_file = '/satellogic-earthview-items-with-matches-1M.parquet'

CLOUD_COVER_THRESHOLD = 0.1 # %
WIDTH = 38
HEIGHT = 38
NUM_CORES = multiprocessing.cpu_count()
SAMPLE = 10  # use 0 to download all, or set to a number to download a samples per zone

def download_images(json_path0, centroid, s2_date):
	...
	if s2_date is not None:
		...
		try:
			...
			return None
	return (dst_path_sentinel2, output_path)

def download_matches(args):
	s2_matches, date, json_path, centroid = args
	if len(s2_matches) > 0:
		...
		if len(s2_matches_filtered) > 0:
			...
			return download_images(json_path, centroid, s2_date)
		return None
	return None

if __name__ == "__main__":
	print("Reading Satellogic with matches items... ", end="", flush=True)
	gdf = gpd.read_parquet(path + data_file)
	print("Done")
	zones = sorted(gdf['zone'].unique())
	for z, zone in enumerate(zones):
		zone_gdf = gdf[gdf['zone'] == zone]
		if SAMPLE > 0:
			zone_gdf = zone_gdf.sample(SAMPLE)
		print("Zone:", zone, f"({z+1}/{len(zones)})", f"({len(zone_gdf)} samples)")
		args = [(item.s2_matches, item.date, item.json_path, item.geometry.centroid) for _, item in zone_gdf.iterrows()]
