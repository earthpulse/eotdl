...
def download_images(json_path, centroid, date):
	...
	if not Path(dst_path_sentinel).exists():
...
for z, zone in enumerate(zones):
	print("Zone:", zone, f"({z+1}/{len(zones)})")
	zone_gdf = gdf[gdf['zone'] == zone]
	sample = zone_gdf.sample(SAMPLES_PER_ZONE, random_state=2025)
	...
	print("Found", sum(results), "matches")
