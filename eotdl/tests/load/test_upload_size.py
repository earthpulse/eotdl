...
def generate_fake_tif_dataset(path: Path, n_tifs: int = 10, tif_size: int = 256, name: str = "test_set"):
    files_to_upload = []
...
def test_load_tifs(setup_mongo, setup_minio, tif_size, n_tifs):
    ...
    with tempfile.TemporaryDirectory(prefix="loadtest_") as tmpdir:
        tmpdir = Path(tmpdir)
        generate_fake_tif_dataset(path=tmpdir, tif_size=tif_size, n_tifs=n_tifs, name=name)
        ...
        with open("eotdl/tests/load/upload_times_tifs.log", "a") as log_file:
            log_file.write(f"{tif_size}x {tif_size} made of {n_tifs} files took {upload_duration:.2f} seconds. ({round(dataset["versions"][0]["size"]/(1024*1024),2)} mb)\n")
