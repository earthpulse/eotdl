...
@with_auth
def download_dataset_prototype(
    dataset_name,
    version=None,
    path=None,
    logger=print,
    ...
    if assets:
        ...
        for row in tqdm(df.iterrows(), total=len(df)):
            for k, v in row[1]["assets"].items():
                href = v["href"]
                if "https://api.eotdl.com" in href:
                    _, filename = href.split("/download/")
                else:
                    filename = href.split("/")[-1] # puede dar problemas si hay dos items con el mismo nombre pero en diferente path
